...
namespace NianticSpatial.NSDK.AR.Samples
{
    public class SitesListManager : MonoBehaviour
    {
        ...
        private Button _useMyLocationButton;

        private const double DefaultLatitude = 0.0;
        private const double DefaultLongitude = 0.0;
        private const double DefaultRadius = 1000.0;
        ...
        private void StartNearMeFlow()
        {
            ...
            CreateInputField("Latitude", DefaultLatitude.ToString("F6"));
            CreateInputField("Longitude", DefaultLongitude.ToString("F6"));
            CreateInputField("Radius (meters)", DefaultRadius.ToString("F0"));
            CreateBackButton("← Back to Mode Selection", ShowModeSelection);
            _useMyLocationButton = CreateButton("📍 Use My Location", UseMyLocation);
            CreateButton("Search Nearby Sites", OnSearchNearMeClicked);
        }

        private void UseMyLocation() => StartCoroutine(UseMyLocationCoroutine());

        private IEnumerator UseMyLocationCoroutine()
        {
            if (_useMyLocationButton != null) _useMyLocationButton.interactable = false;
#if UNITY_ANDROID
            if (!UnityEngine.Android.Permission.HasUserAuthorizedPermission(UnityEngine.Android.Permission.FineLocation))
            {
                UnityEngine.Android.Permission.RequestUserPermission(UnityEngine.Android.Permission.FineLocation);
                // Wait for the dialog to appear, then for the app to regain focus after dismissal
                ...
                if (!UnityEngine.Android.Permission.HasUserAuthorizedPermission(UnityEngine.Android.Permission.FineLocation))
                {
                    if (_useMyLocationButton != null) _useMyLocationButton.interactable = true;
                    SetDetailsText("❌ Location permission denied. Please enable it in app settings.");
                    yield break;
                }
            }
#endif

            SetDetailsText("⏳ Getting your location...");
            // On iOS, Start() triggers the system permission dialog automatically on first use
            ...
            {
                if (_useMyLocationButton != null) _useMyLocationButton.interactable = true;
                SetDetailsText("❌ Could not get location. If you denied permission, please enable Location in app settings and try again.");
                Input.location.Stop();
                yield break;
            }
            ...
            {
                _inputFields[0].text = data.latitude.ToString("F6");
                _inputFields[1].text = data.longitude.ToString("F6");
            }
        ...
        private Button CreateBackButton(string text, Action onClick) => CreateButton(text, onClick);
